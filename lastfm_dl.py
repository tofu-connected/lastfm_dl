...
def write_mp3_tags(filename, artist_name, track_name, thumbnail_data):
    ...
    thumbnail = thumbnail.resize(
        (500, 500), Image.LANCZOS)  # Resize to 500x500
    ...
    for i, track in enumerate(loved_tracks):
        ...
        if youtube_link:
            ...
            audio = sorted(yt.streams.filter(type="audio"), key=lambda s: int(
                str(s.abr).rstrip("kbps")), reverse=True)[0]

            video_title = yt.title

            if not os.path.exists("lastfm_dl"):
                os.makedirs("lastfm_dl")
            ...
            print(
                f"❌ {progress} No YouTube video found for '{artist_name} - {track_name}'")
